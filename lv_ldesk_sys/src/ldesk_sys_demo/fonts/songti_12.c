...
static int binsearch(const uint16_t *sortedSeq, int seqLength,
                     uint16_t keyData) {
...
static const uint8_t *__user_font_get_bitmap(const lv_font_t *font,
  ...
  if (unicode_letter < fdsc->cmaps[0].range_start ||
      unicode_letter > fdsc->cmaps[0].range_length)
    return false;
  ...
  if (unicode_letter == fdsc->cache->last_letter) {
    ...
    i = binsearch(fdsc->cmaps[0].unicode_list, fdsc->cmaps[0].list_length,
                  unicode_letter);
  }
  if (i != -1) {
